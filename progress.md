...
| P5-5 | 四端联调 | ⛔ | 2026-07-25 用户四端真机跑验收，组 3 通过、其余暴露 6 类 bug → **[docs/P5-5测试问题.md](docs/P5-5测试问题.md)**（清单+根因）。**含配额重算回归**：P5-4 触发器依赖 file_size，但元数据 push 把云端 file_size 抹 0（BookUpsertRow.file_size 未 skip_serializing）→ 配额归零。修复顺序：P0-A 配额→P1-B 本地打回云端(merge 无条件 remote)→P1-A 书库不随同步刷新→P2 断网卡死→P0-B 删/加churn 下载失败。待新会话按清单开修 |

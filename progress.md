...
**最后更新**：2026-08-01（v0.3.8 出包：P5-5 六类 bug 全修完——P0-A/P1-B/P1-A/P2 v0.3.7 真机验过，P0-B churn 下载失败+丢封面本版新增，待真机 churn 复验）
**当前阶段**：阶段 5 书籍文件同步进行中 + 多账号本地隔离 epic 已收口。P5-1/P5-2 ✅｜P5-3 🔄（上传/下载核心+R2 往返验证完，待双端 UI 复验）｜P5-4 🔄（配额展示+自动上传开关 ✅，配额双计修复=第二轮待云端部署）｜P5-5 ⏳。多账号隔离 MA-1~8 ✅（MA-9 可选待定）。阶段 4 iOS 已收尾（P4-11 邀测试员=用户）。最新发布 **v0.3.6**。**大量修复待用户真机双端复验**（详见执行日志）。
...
| ID | 任务 | 状态 | 依赖 |
|---|---|---|---|
| P5-1 | 上传队列 | ✅ | 实现+单测+**真实 R2 端到端往返验证通过**（dev Supabase+R2，upload_book_file 上传 42B→R2、download 拉回 SHA-256 校验一致）。run_cycle 元数据后加上传趟：`collect_uploadable`(cloud_state=local)→读文件→`upload_book_file`(sign PUT+直传 R2+PATCH file_key)→synced；配额满停/瞬时失败退避/>100MB 跳过（MVP 单 PUT，登录后自动传）。**e2e 抓到并修真 bug**：sign-url 函数只收 `books/<hash>.pdf`、内部才加 {user_id}/ 前缀，原代码多带 user_id 前缀致 403。踩坑待办：sign 预扣配额致失败重试双计（P5-4 修） |
...
| P5-5 | 四端联调 | ⛔ | 2026-07-25 用户四端真机跑验收，组 3 通过、其余暴露 6 类 bug → **[docs/P5-5测试问题.md](docs/P5-5测试问题.md)**（清单+根因）。**含配额重算回归**：P5-4 触发器依赖 file_size，但元数据 push 把云端 file_size 抹 0（BookUpsertRow.file_size 未 skip_serializing）→ 配额归零。修复顺序：P0-A 配额→P1-B 本地打回云端(merge 无条件 remote)→P1-A 书库不随同步刷新→P2 断网卡死→P0-B 删/加churn 下载失败。**P0-A ✅ 已修**（file_size 改 Option+skip_serializing，元数据 push 不再抹 0；e2e 补场景6「传后再push元数据 配额不变」，连 dev Supabase 实证）；**P1-B ✅ 已修**（merge_remote_books UPDATE 去掉无条件 cloud_state='remote'，文件本地性不被元数据合并改动，+2 回归测试）；**P1-A ✅ 已修**（run_cycle 返回 library_changed，合并有新增/删/改书或下好封面时 emit library-changed→前端已监听自动重刷，组1+组4a 一并解决）；**P2 ✅ 已修**（reqwest 加 connect_timeout 8s 离线快失败+离线文案归一化+文件传输单开 client total 300s+前端 withTimeout 兜底）；**P0-B ✅ 已修**（A 复活清 cover_key/cover_path 让封面重传+B add_books 发 SyncNow 即时重传+C 删除时 PATCH 清云端 file_key/cover_key 状态诚实）。**6 类 bug 全修完**，待真机 churn 复验 |
...
- **2026-08-01（P5-5 修复 · P0-B 删/加 churn 后跨端下载失败+丢封面 ✅）**：查证 delete→revive→重传时序，确认 3 个结构性根因：①**丢封面（永久）**——删除删掉封面 R2 对象，但 `revive_deleted_book_record` 不清 `cover_key`，而 `collect_uploadable_covers` 条件是 `cover_key IS NULL` → 封面永不重传、云端 cover_key 悬指已删对象 → 各端 404；②**revive 发布非原子**——删除从不清云端 file_key，revive 先 push deleted=false 再传文件，中间窗口云端广告 file_key 悬空的书 → 另端下载 404；③**re-add 不发 SyncNow**（remove_book 发、add_books 不发）→ 重传拖到 ≤5min 心跳，首轮失败再 +5min ≈ 10min。**修法 A+B+C**：**A** [lib.rs revive](src-tauri/src/lib.rs:150) 追加 `cover_key=NULL, cover_path=NULL`（当全新书，开书 postProcessBook 重生封面→重传）；**B** `add_books` 加 sync 参数，有 added 就 `SyncNow`（对称 remove_book），窗口从 ≤5min 缩到秒级；**C** `delete_book_file` 删 R2 对象后 PATCH 云端行 `file_key=null, cover_key=null`（状态诚实，与配额触发器自洽）。强化 `readd_same_hash_revives_deleted_book` 断言 cover_key/cover_path 清空。clippy 干净、55 单测全绿。**待真机 churn 复验**（多端删/加几轮验下载+封面）。
- **2026-08-01（P5-5 修复 · P1-A 书库不随后台同步刷新 ✅）**：根因=后台同步引擎改了本地库（新增/删书、下好封面）后前端书库不响应式重刷；`library-changed` 事件此前只在切账号发（MA-6），常规同步变更不发。**组1**（登录后同步下来的书/封面不显示、切排序才出）+**组4a**（Mac/Win 删的书不消失）同源。**修法（纯 Rust 侧，前端已监听无需改）**：`run_cycle` 返回 `bool library_changed`——`merge_remote_books` 的 `inserted+updated>0`（含墓碑翻 deleted）或 `download_pending_covers` 落盘数 `>0` 即为真；两个 run_cycle 调用点（RefreshNow / 心跳周期）在成功且 changed 时 `app.emit("library-changed")`。前端 [Library.tsx:248](src/components/Library.tsx:248) 早已 `listen→reload()`。进度合并不计入触发（翻页频繁、开书读 DB 即最新），避免书库无谓重载。`download_pending_covers` 返回类型 `()→usize`。选 A（核心自动刷新，桌面手动入口从简不做）。clippy 干净、55 单测全绿。
- **2026-08-01（P5-5 修复 · P2 断网操作卡死转轮 ✅）**：根因=reqwest client 只设 `.timeout(30s)` 总超时、**无 connect_timeout**，飞行模式每个请求朝 30s 漂移；一次下载/刷新是多个串行请求叠成分钟级，且引擎单线程——卡死的下载把排队的刷新也堵住 →「重新联网也不恢复」。前端转轮在 finally 会清，但要等 promise 落定（60s/180s channel 兜底）。**修法（①+②+A2+B1）**：① [sync_supabase.rs](src-tauri/src/sync_supabase.rs) client 加 `connect_timeout(8s)`——离线连接阶段秒级快失败（决定性杠杆）；②`friendly_network_message` 把 `is_connect()/is_timeout()` 归一化为「网络不可用，请检查网络连接后重试 / 网络响应超时」友好文案（`map_network_error`+新增 `map_transfer_error` 共用）；**A2** 文件传输单开 `file_client`（connect_timeout 8s + total 放宽 300s），5 处 R2 PUT/GET/DELETE/封面 转用它，大书慢网不被 30s 误杀（顺手根治潜在隐患）；**B1** 前端 `withTimeout`（下载 25s/刷新 20s）安全网，到点停转+提示，底层 invoke 继续跑完无害（下载稍后成功→引擎 emit library-changed 自愈）。clippy 干净、tsc 干净、55 单测全绿。**断网手感待真机验**。
- **2026-08-01（P5-5 修复 · P1-B 已下载书被打回云端重下 ✅）**：根因=`merge_remote_books` 的 UPDATE 无条件 `cloud_state='remote'`（sync_engine.rs）。另端 push 进度/元数据 bump updated_at → 本端 pull→merge UPDATE → 把本已 synced（文件在本机）的书打回 remote → 显示云端、点开重下（内容寻址同路径覆盖，无重复文件但白下一次）。**修法**：UPDATE 分支删掉 `cloud_state='remote'` 赋值——文件本地性（local/uploading/synced/remote）取决于本机是否真有文件本体，纯元数据合并无从得知，不该改它；INSERT 分支仍置 remote（新书来自云端、本机无文件）是对的。**测试**：原 `merge_books_远端新数据覆盖本地旧数据` 的 `cloud_state=='remote'` 断言本身就是 bug 固化，改为断言保持 'local'；+2 回归测试（synced 书元数据更新后仍 synced、remote 书更新后仍 remote）。clippy 干净、55 单测全绿。
- **2026-08-01（P5-5 修复 · P0-A 配额归零 ✅）**：根因=P5-4 触发器把 `bytes_used` 算成 `SUM(file_size WHERE file_key NOT NULL AND NOT deleted)`，但 `collect_dirty` 恒送 `file_size:0` 且 `BookUpsertRow.file_size` 无 `skip_serializing_if`——每次元数据 push（翻页 bump updated_at 即触发）都把云端已 PATCH 的真实 file_size 覆盖成 0 → SUM 归零 → 配额清零；删书只是触发一次重算暴露它。**修法**（与 file_key/cover_key 同套路，让 file_size 归 upload_book_file 的 PATCH 独占）：`CloudBook.file_size`→`Option<i64>`、`collect_dirty` 送 `None`；`BookUpsertRow.file_size`→`Option<i64>`+`skip_serializing_if=Option::is_none`（元数据 push 恒不带此列，merge-duplicates 不动缺失列）；`BookPullRow→CloudBook` 用 `Some(self.file_size)`；books.file_size 有 `default 0` 兜首次 INSERT。**e2e 补场景6**：`quota_accounting_five_scenarios` 加「上传后再 push 元数据 → 配额仍=flen」（正是旧代码归零、原 e2e 漏测的路径），mk_book 改送 file_size:None 模拟真实 push。clippy 干净、53 单测全绿。**e2e 需连 dev Supabase 手跑验证**（会注册新账号+传 R2，待用户或授权后跑）。P1-B 待续。

- **2026-07-25（P5-5 四端真机验收：暴露 6 类 bug，整理清单待修）**：用户跑验收清单，组 3 多账号隔离通过 ✅，其余暴露问题→[docs/P5-5测试问题.md](docs/P5-5测试问题.md)（含代码级根因）。要点：**P0-A 配额归零=P5-4 配额重算回归**（触发器依赖 file_size，但 collect_dirty 恒送 file_size:0 且 BookUpsertRow.file_size 未 skip_serializing → 每次元数据 push 抹云端 file_size 为 0 → SUM 归零；e2e 没测「上传后再 push」故漏）；P1-B 已下载书被打回云端重下（merge_remote_books UPDATE 无条件 cloud_state='remote'，代码确认）；P1-A 书库不随后台同步刷新（library-changed 只在切账号发，常规同步变更不发，桌面还无下拉刷新）；P2 断网操作转轮卡死~3min（超时兜底缺失）；P0-B 删/加churn 后跨端下载失败 10min、登录重来自愈+个别丢封面（需复现查证）。**本轮只总结不改**，待新会话按修复顺序开工。
- **2026-07-25（P5-4 第二轮：配额双计修复，TDD + 已部署）**：用户要求先设计测试再实现、测 5 场景全过、测试自建账号做上传。**TDD**：新增 `#[ignore] quota_accounting_five_scenarios`——sign_up 全新账号（干净基线）跑 5 场景，每步比对实测/期望，跑完统一断言。**改前跑=红**，精确暴露 5 bug：签了没传=1000、上传=1051、重传=1102、封面=1129、封面重传=1156、删除=1156（应为 0/51/51/51/51/0）。**修复**：① 迁移 `20260725000001_quota_recompute.sql`——`recompute_user_quota()`(SECURITY DEFINER) 触发器，在 books 的 file_key/file_size/deleted 变化时把 bytes_used 重算为 `SUM(file_size WHERE file_key IS NOT NULL AND deleted=false)`；一次性回填修正历史虚高。② sign-url 函数：PUT 去掉预扣只留超额检查；delete 去掉 HEAD 回扣只留签发（回落改由 deleted=true 触发器完成）。**部署**：`supabase db push`（迁移+回填）+ `supabase functions deploy sign-url` 均成功。**改后跑=绿**：5 场景实测 0/51/51/51/51/0 全对；原 real_r2_roundtrip 文件+封面往返无回归。决策：封面不计入配额（file_size 只记正文）。回填顺带修正了用户真实账号的虚高配额。
...
- **2026-07-24（书卡状态标签 UI）**：用户反馈"不知道书本什么状态"（云端书封面是占位、无提示）。书卡 meta 行改为三态明确标签：**☁ 云端**（remote，灰，点击下载）｜**转圈 spinner + 下载中**（下载进行中，复用 account-spin 动画；下载中再点弹"《…》正在下载…"提示，替代原静默返回）｜**✓ 本地**（local/synced，柔和绿 #3a9a63，已在本机可读）。模拟器注入 local/remote/synced 三态书实拍确认三标签正确渲染（云端灰云、本地绿勾）。改 Library.tsx（tryOpenBook 下载中点击加提示 + meta 三态条件渲染）+ styles.css（.meta-status/.meta-spinner/.meta-local/.meta-cloud）。tsc/vite build 干净。**纯前端，需重新出包用户方能真机见效**。
- **2026-07-24（修 iOS 书卡 ⋯ 菜单无法再点关闭）**：用户反馈点开 ⋯ 菜单后再点同一个 ⋯ 不能取消弹窗。根因：⋯ 按钮 onClick 的 `e.stopPropagation()` 挡掉了「点外部关闭」的 window click 监听，而 `openTouchMenu` 每次无条件 `setCtxMenu(...)` 重开，故第二次点只是重新打开、看似无反应。修复=`openTouchMenu` 改 toggle：`ctxMenu?.book.id === book.id` 则 `setCtxMenu(null)` 关闭，否则正常打开；点另一本书的 ⋯ 仍切换，点菜单外仍由原 window 监听关闭。单处改动（Library.tsx），纯前端，不影响桌面右键（走 onContextMenu 不经此函数）。tsc/vite build 干净。自动 tap 通道受限（MCP xcode-select 误报 + System Events 无辅助功能授权），交互二次点关闭待用户真机/模拟器确认。CI 三 job 全绿（[run 30142858469](https://github.com/Topia99/Shelf-Book-Reader/actions/runs/30142858469)）。
